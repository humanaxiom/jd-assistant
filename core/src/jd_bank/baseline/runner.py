...
def _stable_reason(exc: Exception) -> str:
    ...
    ``extract._extract_doc`` writes the bytes to a ``NamedTemporaryFile`` before handing
    them to antiword, so a failure message names it::

        antiword failed: /tmp/tmp0k444cks.doc is not a Word Document

    That path is freshly random every run. Left in, it would (a) make ``rows.jsonl``
    byte-different on two runs over the *same* archive — destroying the reproducibility
    an audit trail is made of — and (b) shatter the grouped skip ledger, so files that
    failed for one identical reason would each report as their own unique reason with a
    count of 1. Measured over the full archive: 11 files carry a reason that named the
    temp path, and the largest group that would have shattered is 9.

    The path is an artefact of **our own** temp-file handling and says nothing about the
    JD, so scrubbing it discards no evidence. Everything else in the message survives
    verbatim.
    """
    return _TMPFILE.sub("<tmpfile>", f"{type(exc).__name__}: {exc}")
