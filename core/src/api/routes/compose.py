"""JD Builder — compose routes (Phases 5.1 + 5.5 + 5.4).
...
to the review queue (5.6). ``/validate`` is pure and DB-free. The LLM / embedding /
Neo4j clients are built via injectable factories (so tests substitute fakes and no
network is hit) — the content clients are egress-guarded at construction (NN #5) and
