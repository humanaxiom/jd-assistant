...
follow-ups:** LLM-enrich a batch and/or full-archive seed (for real prose / the complete bank) —
**a validation batch is IN PROGRESS** (21 `--limit 600`-scope drafts through the 4.2a rewrite + 4.2b
audit on `gpt-oss:120b`/`aria-gb10-2`; single-transaction, silent, commits at end); tighten
...
orphaned). Repo-wide staleness swept: decision-register count corrected to **190** (was 119/123/166 in
various docs), the 874-cohort grade spread corrected to **81 A / 551 B** (was 82 A / 550 B) in the
packet, baseline README, change-plan, and the register source (regenerated; `register-check` green),
...
also wired into CI). **190 decisions, all `open` — SFU HR has ratified nothing yet, but the packet
is now written and the numbers in it are corrected: `docs/decisions/HR-REVIEW-PACKET.md`.**

Provenance (at 190 entries): **99 our-invention · 72 hris-calibration · 19 SFU-rulebook**. The entire approval
...
`current_default` drifts from the live value; a param on the 251-item decision surface is
