...
| Security | 🔴 **P0 — the JSON API (`/jd-bank`) and legacy harness routes carry NO auth gate and there is no middleware.** With CAS on, an unauthenticated `POST /jd-bank/review/{id}/approve` reaches the review service and is refused only by business rules — on a gate-clean DRAFT it would **publish** (breach of NN #1). `service.py` also writes the attacker-supplied `reviewer_id` into the hash-chained audit log, so a forged actor is attested intact (NN #6). No test pins it. Plan: `docs/tasks/architecture-review-response-2026-08-07.md` | open | M |
