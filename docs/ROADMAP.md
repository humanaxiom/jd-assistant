...
> **▶ Order of work as of 2026-08-13** (full state: `HANDOFF.md` §START HERE):
> ~~P0.0 navigability~~ ✅ **DONE ([#88](https://github.com/humanaxiom/jd-assistant/pull/88))**
...
> → ~~P0.1b-ii, ALL of it~~ ✅ **DONE** ([#91](https://github.com/humanaxiom/jd-assistant/pull/91) login CSRF + open redirect · [#92](https://github.com/humanaxiom/jd-assistant/pull/92) `/ready` amplification + `GraphMemory` egress)
> → ~~P1.3 tier the register~~ ✅ **DONE** — the HR ask is **65 settings, not 197**
> → **P1.2 harmonization provenance ⟵ NEXT**. The HR docs behind all of this
> merged first as [#86](https://github.com/humanaxiom/jd-assistant/pull/86).
>
> 🔴 **`sfuai.ca:7000` is INTERNET-FACING (confirmed 2026-08-13).** The app is served over
> plain http at a public name in `environment=development`, so P0.2's fail-closed guard is
> inactive on the deployment that most needs it, and session cookies and CAS tickets cross
> the open internet in the clear. The data stores are bound to loopback now; TLS at the
> edge and the production compose profile are what P0.4 is. Localhost sign-in **works
> again** — P0.3's allowlist serves both origins at once.
>
...
| Ops | ✅ ~~Backup + reindex runbooks~~ **DONE 2026-08-13** — [`docs/runbooks/backup-and-restore.md`](runbooks/backup-and-restore.md) + [`docs/runbooks/reindex.md`](runbooks/reindex.md), linked from the operator guide, **every step executed against the live stack before being written**. Found a 🔴 silent data-destroying restore path: `pg_restore --data-only` into a migrated database took 1,804 canonical JDs → 0, 12 user roles → 0, 1,810 chained audit rows → 0 **and exited 0**. The blessed path restores the audit chain byte-identical. **Rate/size limits remain open**, and the review found only one necessary: `/ready`'s single-flight bound, shipped in P0.1b-ii ([#92](https://github.com/humanaxiom/jd-assistant/pull/92)) — so what is left here is an unevidenced want, not a known gap | runbooks ✅ **done**; limits open | S |
...
| Ops/Security | ✅ **P0.3 — DEPLOYMENT ORIGINS: DONE (PR [#89](https://github.com/humanaxiom/jd-assistant/pull/89), 2026-08-13).** `ALLOWED_SERVICE_ORIGINS` ships: the origin a browser arrived on is honoured only if it is on the list, exactly — else the static fallback, **never** the header. `cas_service_from_request` retired and refused at load in every mode; data ports bound to loopback. **The exposure question is ANSWERED — internet-facing** — which promotes the production compose profile to **P0.4**. *What follows is the defect as raised.* Raised 2026-08-11 when the system was first reached from outside the dev box (`sfuai.ca:7000` → `25800`): CAS authenticated, then returned the browser to **`localhost:25800`**, because the return origin is a single static setting. Repointing it worked and is a **workaround, not a design** — one value cannot serve localhost, the forward, and a future hostname. The existing alternative (`cas_service_from_request`, from `X-Forwarded-Host`) is the header-injection vector P0.2 refuses in production, so the options today are *rigid* or *exploitable*. Build the third: **derive the origin, validate against an allowlist** (shape it like `allowed_inference_hosts`), force https in production, fall back to the static value — never to the header. **Part 2, still unanswered:** the app is now reachable at a public name over **plain http** in `development`, so P0.2's guard is inactive, DB/Neo4j creds are the committed defaults, and the data ports (`25432`/`25474`/`25687`/`25379`) publish on `0.0.0.0` — **a larger exposure than the CAS bug that surfaced it.** Task: `docs/tasks/P0.3-deployment-origins.md` | ✅ **done** | S/M |
| Ops/Security | ✅ **P0.4 — THE PRODUCTION POSTURE: BUILT (PR [#90](https://github.com/humanaxiom/jd-assistant/pull/90), 2026-08-13).** `docker-compose.prod.yml` — no reloader, no source mount, healthcheck, restart policy, credentials with **no defaults** (compose refuses to render without them). **Verified by deploying it**, not by reading it: a safe posture serves `{"status":"ok"}`; an unsafe one reads `Restarting (3)` / `unhealthy` and never comes up. A standalone file rather than an overlay **because compose merges volume lists** — measured: an overlay declaring `volumes: []` still renders the base `./core:/app` bind, so an overlay cannot make the image be what runs. ⚠️ **Available, not in force:** the live demo still runs the dev stack, and **TLS at the edge remains open** — that is now the last exposure and it needs a person, not a commit. *What follows is the defect as raised.* Split out of P0.1b and **promoted to P0 by P0.3's answer**: `sfuai.ca:7000` is internet-facing, so the app is public over plain http in `environment=development` and P0.2's fail-closed guard is inactive on the one deployment that most needs it — session cookies and CAS tickets cross the open internet in the clear. Needs a **production compose profile**, TLS at the edge, real secrets, `SESSION_COOKIE_SECURE=true`, `ENVIRONMENT=production`. ⚠️ **The trap:** `api` runs `uvicorn --reload`, so a settings refusal inside the reloader presents as **"Up but serving nothing"** rather than a dead container — the P0.2 failure mode one layer down. Detail: `docs/tasks/P0.3-deployment-origins.md` §Part 2 | ✅ **built, not in force** | M |
| Security | ✅ **P0.1b — THE WHOLE BUNDLE IS CLOSED (2026-08-13).** Login CSRF + the open redirect on `next` ([#91](https://github.com/humanaxiom/jd-assistant/pull/91)) — one code path, one fix: `next` pinned to a local path, and an HttpOnly login-state cookie binding the CAS round trip to the browser that started it (an attacker can get a state of their own; they cannot set a cookie in the victim's browser — that comparison is the control). The production compose profile ([#90](https://github.com/humanaxiom/jd-assistant/pull/90)). `/ready` amplification + `GraphMemory` egress ([#92](https://github.com/humanaxiom/jd-assistant/pull/92)) — single-flight + 2s TTL, **peak Postgres backends under 300 concurrent: 2, was 13 at 200**. *What follows is the bundle as raised.* login CSRF (the `GET /cas/validate` leg mutates — provisions a user, may grant ADMIN, mints a session — and no token can exist before the session it belongs to), the open redirect on `next`, `/ready` amplification (200 concurrent → 3.00s vs `/health` 0.43s), a production compose profile, and `GraphMemory.__init__` building an `AsyncOpenAI` outside the egress guard. Plan: `docs/tasks/architecture-review-response-2026-08-07.md` §6 | ✅ **done** | M |
| Security | ✅ ~~🔴 **P0 — the production startup guard `settings.py` claims does not exist**, and `cas_enabled=False` returns a transient **admin** before any cookie is read; `.env.example` has no auth keys~~ **CLOSED BY P0.2 ([#83](https://github.com/humanaxiom/jd-assistant/pull/83)) — this row was STALE and is corrected here (verified 2026-08-13: the `model_validator` exists in `settings.py`, and `.env.example` carries the auth keys).** A backlog that lists closed work as open costs a re-investigation every time it is read | ✅ **done** | — |
| Governance | ✅ **P1.3 — TIER THE DECISION REGISTER: DONE (2026-08-13).** Every entry carries a required `tier`; the generated register leads with **"Your decisions"**. Measured **65 `hr_policy` · 49 `hr_informed` · 83 `technical`** — the estimate below said ~55–60 and was close. Three properties make it more than a label: **`tier` has no default** (a new parameter cannot be filed by omission — how 197 undifferentiated entries accumulated); **nothing became unregistered** (every build-breaking check still walks all 197 — the *audience* changed, not the coverage); and **`comparison`/`hay_signals` may never be `hr_policy`**, pinned by a test and **proved by mutation** in both directions. `rules_version` unmoved — the register is unhashed and no default changed. *What follows is the item as raised.* 57 of 197 entries sit in the `comparison`/`hay_signals` adapter ADR-007 disclaims as *not* classification, and 8 are embedding knobs; only ~55–60 touch the approval bar. Add `hr_policy`/`hr_informed`/`technical` so HR sees ~60 real calls, not 197 | ✅ **done** | S/M |
| Pipeline | **31.9% of the archive (4,630 JDs) has no parsed `employee_group` — MEASURED 2026-08-13, and it is NOT the parser's residual.** Over a 300-file random sample of those 4,630: **91.3% contain no group token anywhere in the text** and **1 in 300 carries an `Employee Group:` label the parser missed**. The documents do not state the fact. The filename recovers **193 of 4,630** (4.2% of the residual, 1.3% of the archive) and that is the ceiling. **The 8.7% that mention a token are a trap, not an opportunity** — they read *"Supervises one full-time CUPE support staff person"*, i.e. JDFN roles that supervise CUPE, so classifying on a loose match would manufacture exactly the category error HR-143/HR-194 exist to prevent. **No code shipped, deliberately:** a parser change needs a `parser_version` bump, and bumping it without immediately re-parsing all 14,565 files leaves every layer querying a version with no rows — an apparently empty Bank. The bump and the re-parse must ship together, by a person. **Both stated premises are corrected:** it is not the parser's residual, and it is not dependency-free — closing it properly means the **HRIS**, which is the HR-blocked export/FIPPA thread. Evidence: [`docs/decisions/employee-group-residual-2026-08-13.md`](decisions/employee-group-residual-2026-08-13.md) | **measured — HR/ops-gated** | — |
| Pipeline | **🟠 51 canonical JDs have BOILERPLATE PROSE AS THEIR TITLE — found 2026-08-13 by 8.3b, recorded not fixed.** Their `content.title` is longer than 80 characters and reads like *"We are Canada's engaged university, defined by its dynamic integration of…"* or the opening sentence of the position summary. Exactly one is **PUBLISHED** (measured), and a reviewer had already noticed: the archive's first `version = 2` draft corrects *"In collaboration with the Communications Officer, the Multimedia Coordinator supports…"* to **"Multimedia Coordinator"** — the word-level diff of that real edit reports Title `−24 words` and Identification `+4 words`, so the title fix is one of two things that edit did. The defect was invisible while each JD was read alone and became obvious the moment the **8.3b sidebar put related titles side by side** — a list is a different instrument from a page. **Not fixed here, and for the same reason as the `employee_group` residual:** the fix is in the *parser*, a parser change needs a `parser_version` bump, and bumping it without immediately re-parsing all 14,565 files leaves every layer querying a version with no rows — an apparently empty Bank. Bump and re-parse must ship together, by a person. **A title is not cosmetic here** — it is what the Builder searches, what the library lists, and what the sidebar shows a reviewer | open | M |
| UX | ✅ ~~**Submit → 403.** The draft commits, then redirects to a reviewer-only page; the default new-user role is `author`, so this is the default first experience. No author-scoped status route exists~~ **CLOSED BY P0.0 ([#88](https://github.com/humanaxiom/jd-assistant/pull/88), which absorbed P1.1) — STALE row, corrected 2026-08-13:** `routes/drafts.py` serves `/my-drafts` and submit redirects there | ✅ **done** | — |
| Review | ✅ **P1.2 — HARMONIZATION PROVENANCE: DONE (2026-08-13).** The review page gains a **"How this draft was assembled"** panel. **The item understated the defect: `merge_provenance` has been computed and persisted in `change_log` since Phase 4.1 and was read by NOTHING** — not this page, not the library — so skill frequency, duty coverage, contributors and flags were all invisible too, not just the bar. The bar record did not exist at all and is new: `SeniorityBarChoice` records the policy actually applied, the chosen bar and **what every member stated**, so the panel can say *"9 of 10 sources stated a different one"*. **A silent member is not an overruled one** — `None` (stated no bar) is deliberately not counted as dissent, or the panel would overstate the disagreement it is asking the reviewer to weigh. No threshold anywhere: `disagreed` is a property of the members' own statements, so **no register entry is earned**. Advisory and read-only — it changes no verdict (NN #1/#3). Mutation-proved, and the panel degrades to absent (never a 500) on a malformed packet. *What follows is the item as raised.* A reviewer sees a raised education/experience bar with no indication that most sources stated lower (`seniority_bar_policy: max`, HR-175; ~4.3% of clusters). The NN #1 control cannot rule on what it cannot see | ✅ **done** | M |
...
| Chore | ~~`jd_core → jd_bank` import edge~~ ✅ **CLOSED BY [#100](https://github.com/humanaxiom/jd-assistant/pull/100)** — `store.py` moved to `jd_bank/ingest/parse_store.py` and `_KNOWN_CORE_TO_BANK_IMPORTS` is now an empty `frozenset()`, so the ratchet fails if one comes back. **The row itself was the stale half of #100's own sweep** — verified 2026-08-13: no `src.jd_bank` import remains anywhere under `core/src/jd_core/` | ✅ **done** | — |
| Chore | **`get_session` still lives in `api/main.py`, not the `api/deps.py` shim** — genuinely open, and split out of the row above rather than closed with it. Verified 2026-08-13: `get_session` is defined in `core/src/api/main.py` and imported from there by **8 route modules** (`admin`, `auth`, `compose`, `compose_ui`, `drafts`, `jd_bank`, `library`, `ui`), while `api/deps.py` exists alongside it. **⚠ CORRECTED 2026-08-13: the import cycle is not "waiting to be re-introduced" — it is LIVE, and it bit three times while building 8.3c.** `routes/ui.py` imports `get_session` from `src.api.main`, which imports `routes.ui` to mount the router, so `import src.api.routes.ui` **as the first import raises** `ImportError: cannot import name 'router' from partially initialized module`. Every existing suite hides it by importing `src.api.main` first, which is why it has gone unnoticed. `test_gate_jump_links.py` carries an explicit `import src.api.main` with a comment saying why, rather than papering over it. Moving `get_session` to `api/deps.py` is the fix | open | S |
| Docs | ✅ ~~Refresh HANDOFF.md for the auth/RBAC + operator-guide work~~ **DONE** — HANDOFF has been refreshed repeatedly since, most recently for the merged state ([#96](https://github.com/humanaxiom/jd-assistant/pull/96)) | ✅ **done** | — |
