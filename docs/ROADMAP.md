...
|---|---|---|---|
| UX | 🔴 **P0.0 — NAVIGABILITY: no dead ends anywhere.** Found live during an HR demo: `http://localhost:25800` answers `{"detail":"Not Found"}`, and so does `/jd-bank/ui`. The app only serves deep paths, a raw JSON error is what a non-technical user sees, and nothing tests that a rendered link resolves. **Fix the class, not the instances** — a landing route, human-readable error pages, and a crawl test that fails when any rendered `href`/`action` 404s. Task: `docs/tasks/P0.0-navigability.md` | open | S/M |
