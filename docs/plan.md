...
| **P3g** | ⚠ **THE CODE HALF IS DONE. What remains is RUNNING it — and that needs the GPU** | small, blocked | 🔴 **This row was stale and said so for two weeks.** MEASURED 2026-08-29: `(:JDRole)` held **1,797 vectors against 2,500 roles — 703 invisible to Builder search**, and `(:JDDocument)` is embedded at `jd_segmenter_v2`, SIX parser bumps stale. ✅ **The staleness check SHIPPED** in `ba196b9` (#180) — `make smoke` now fails on exactly this, which is why smoke is deliberately red. ✅ **Isolate-and-count SHIPPED** in the same PR: one over-long role no longer aborts the pass. ✅ **The truncate half DONE 2026-09-11** — a rejected role was still counted-and-absent, with no vector and invisible to Builder search; it now walks the **HR-193 fallback ladder** (`embeddings.max_chars_fallback`) exactly as the DOCUMENT runner has since Phase 3.2, and is embedded from the first re-cut the server accepts, counted as `roles_backed_off`. The full-text `text_sha256` is kept as the node's identity, so an unchanged corpus still re-embeds nothing. ⚠ Same PR: `roles_rejected` was in the JSON summary and **NOT in the printed line** — a dropped role was invisible to anyone watching the terminal. Both counts now print. 🔴 **WHAT IS LEFT IS NOT CODE:** `make embed` + `make embed-roles` have to actually RUN to rebuild the index and turn smoke green, and they need `aria-gb10-2` — **which the JDFN producer pass is holding until ~2026-09-13.** Do not run them concurrently; queue them for when the pass lands. |
