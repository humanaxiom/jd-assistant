...
| **P3h** | 🔴 **NEW — 53 titles are a PARAGRAPH, not a title. Found by measuring P3c** | small–med | The P3c probe's `incumbent` cue matched two titles that are whole sentences, which turned out to be the tip of it. MEASURED (§8f): **53 documents (15 distinct) have a title clipped at the model's 200-char cap** — the parser fed a position summary into `title` and pydantic truncated it mid-word — plus **299** opening with a prose word (`Reporting to the Manager, Linguistics, and supervised by…`), 130 over 120 chars. ⚠ **53 is the hard floor, 299 the upper bound.** Unlike P3c this needs no name detection: "the title field contains a sentence" is decidable from length and shape, and it is **53× bigger than the defect that exposed it**. Fix at the parser (the title extractor is taking the wrong cell/line), not by post-hoc trimming — a truncated paragraph is a confident wrong value, and §8d's own rule is to refuse back to the sentinel rather than ship a fragment. |
