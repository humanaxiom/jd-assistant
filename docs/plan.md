...
| **P3g** | 🔴 **Nothing rebuilds the vector index, nothing notices — and `embed-roles` ABORTS on one oversized role** | small–med | MEASURED 2026-08-29: `(:JDRole)` held **1,797 vectors against 2,500 roles — 703 invisible to Builder search**, and `(:JDDocument)` is embedded at `jd_segmenter_v2`, SIX parser bumps stale. Neither errors: the query has no version filter, so it ranks on text the parser no longer produces. Neo4j is a DERIVED index and no gate checks it. Either add a staleness check to `make smoke`, or make a producer/parse run say out loud that `make embed`/`make embed-roles` is owed. 🔴 **AND `make embed-roles` is not resumable past a bad row:** it died with `EmbeddingBadRequestError: the input length exceeds the context length` — ONE role longer than `nomic-embed-text` accepts aborts the whole pass, so every role after it stays unembedded. It reached **2,152 of 2,500** (the Bank says so; the make target just said `Error 1`). Isolate-and-count per role, as the producer does for a failed rewrite, and truncate or chunk the input. |
