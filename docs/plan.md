...
| **BGL-3** | **The `sfuai.ca:7000` forward has stopped delivering** | Gateway rule still visibly configured, DNS still matches the live WAN IP, nothing external arrives — 15 external probes timed out and the app logged none. No hairpin, so it cannot be tested from inside. Reach the box over Tailscale meanwhile. Needs someone at the gateway; **moot once the DC boxes are up**, which is why it is here and not in a track. |
