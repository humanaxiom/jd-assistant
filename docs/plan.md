...
| **P5** | 🔴 **NEW — `--resume` cannot resume a RE-BASELINE, and HANDOFF told the next session to use it** | small | MEASURED 2026-09-12, before restarting the killed JDFN pass. `--resume` sets `skip_llm_written`, which skips any cluster where `draft_has_rewritten_prose(change_log)` = `rewrite_ran AND NOT rewrite_failed`. That means **"prose landed"**, not "this pass produced it" — and every JDFN draft already held a successful rewrite from an earlier run. Of the **1,048 clusters still owing the re-baseline, `--resume` would process 9 and skip 1,039**, then exit green in ~40 minutes with a summary reporting success. ⚠ It is the `llm_enabled` defect of `ba196b9` one level up: the predicate answers a *different question* from the one the flag's name promises, and the failure mode is again a silent no-op that looks like completion. **Fix: skip only clusters already refreshed under the CURRENT rulebook/prompt stamp** (the stamp is already on the row), so a re-baseline sees its own work and nothing else. Until then a re-baseline must run WITHOUT `--resume` and pay to redo everything already done — ~16 h of the current 37 h run. |
