...
- **We ship:** `title` → ["Department's Position Title", 'Department’s Position Title', 'Departments Position Title', 'Department Position Title', 'Position Title']; `department` → ['Department Name']; `position_number` → ['Position Number(s)', 'Position Number']; `grade` → ['Classification & Grade Approved', 'Classification and Grade Approved']
- **Configured in:** `wjq.yaml` → `wjq.id_labels`
- **Where the default came from:** we chose it
- **Why it matters:** These inline labels are read to populate `title` / `department` / `position_number` / `grade`. `_extract_label` matches a cell EXACTLY (case-insensitive, colon-stripped), so a spelling missing from this list matches NOTHING — the field is not degraded, it is absent. 🔴 CORRECTED 2026-08-29, and the correction was large. This entry previously said "many `.doc` renders omit it, so `title` falls back to `Untitled Position`". That was an ASSUMPTION about the archive and it was wrong. MEASURED against the raw source files: **47.6% of CUPE documents (2,046 of 4,300) had no title at all**, against **0.0% for every other bargaining unit** — so it was never a general title problem, it was this list. The label was not omitted; it was spelled POSSESSIVELY ("Department's Position Title", plus the curly-apostrophe and apostrophe-less variants), which this list did not carry. Of 500 placeholder documents read from disk, 17.4% carry a title under a spelling now listed, with the value correctly in its own next cell. ⚠ 1,395 of those placeholders were ALREADY inside drafts.
