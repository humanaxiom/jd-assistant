...
- **Where the default came from:** our invention
- **Why it matters:** The model carries these as presence BOOLEANS, not boilerplate TEXT (bank/render.py), so a draft asserting "present" still cannot render the mandated paragraphs — a re-parse trips SFU-COMP-ABOUT / the footer gates regardless. `or_across_members` asserts present iff ANY member had it — the honest reading. MEASURED (1,801 JDFN clusters): members frequently DISAGREE on presence — mixed in 106 (about_sfu) / 129 (employment_equity) / 179 (territorial) clusters — so the OR really engages, and asserting present iff any member carried it is the defensible reading of a merge. ⚠ This flips to `all_present` ONLY IF HR ratifies composer auto-insert of the footer (an HR-pending decision — see the HR-REVIEW-PACKET footer question); shipping `all_present` now would pre-empt that ruling and make the draft claim compliance it does not carry. Kept, pending HR's footer ruling.
