...
- **If it changes:** Moves `rules_version` (rule_catalog.yaml is hashed). Adding `wjq` to these three rules re-applies them to all 4,440 CUPE JDs, where they currently fire on ~100% — restoring roughly a third of the score gap Phase B closed. Pinned by `test_rule_applies_to_template.py`, which fails if the measured JDFN-only set moves.
