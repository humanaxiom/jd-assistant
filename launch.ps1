...
.EXAMPLE
    .aunch.ps1 -Rebuild -ArchivePath C:\repos\hris\fixtures\SFU_JDs
...
# ── Is every service actually RUNNING? ──────────────────────────────────────
# `up --wait` returns when the services it STARTED are healthy. It says nothing about a
# service that started fine and died later — and `docker compose ps` printing "Exited
# (1)" in a table nobody reads is not a check.
#
# 🔴 This is why it exists: editing docker-compose.yml makes the next `docker compose
# run` recreate dependencies, which drops the worker's Redis connection and arq exits.
# On 2026-08-28 `jd-bank-worker-1` sat "Exited (1)" for NINE HOURS with the stack
# otherwise green and nobody noticed. A launch that reports success over a dead service
# is worse than one that fails.
# `ps -a`, not `ps`: without it a stopped service simply vanishes from the listing and
# reads as "no container", which is exactly the wrong description of the case this check
# was written for — a container that ran and then EXITED.
$notRunning = @()
foreach ($svc in $Services) {
    $state = (& docker compose ps -a --format '{{.Service}} {{.State}}' |
        Where-Object { $_ -match "^$svc " }) -replace "^$svc ", ''
    if (-not $state) { $notRunning += "$svc (no container)"; continue }
    if ("$state".Trim() -ne 'running') { $notRunning += "$svc ($($state.Trim()))" }
}
if ($notRunning) {
    Write-Warn "NOT RUNNING: $($notRunning -join ', ')"
    Write-Info "  Logs:  docker compose logs $($notRunning[0].Split(' ')[0])"
    Write-Info '  This stack is NOT fully up, whatever the rows above say.'
}
else {
    Write-Ok "All $($Services.Count) services are running"
}
...
if ($casOn) {
    ...
    Write-Info '  For frictionless dev access instead: .aunch.ps1 -NoCas'
}
else {
...
Write-Info '  .\teardown.ps1                               stop, keep the data'
Write-Info '  .\teardown.ps1 -Volumes                      ...and DISCARD the Bank'
Write-Host ''
